...
public class Player : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        anim = GetComponentInChildren<Animator>();    // 단일 하위 컴포넌트만  가져옴
        meshs = GetComponentsInChildren<MeshRenderer>();  // 복수 하위 컴포넌트들 가져옴!(MeshRender를 포함한 자식 컴포넌트들을 싹다 가져옴)


        if (!PlayerPrefs.HasKey("MaxScore"))
        {
            PlayerPrefs.SetInt("MaxScore", 0);  // 유니티에서 제공하는 간단한 저장 기능
        }     
    }
